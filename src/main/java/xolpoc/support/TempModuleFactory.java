...
/**
 * Temporary ModuleFactory override that never returns null from {@link #determineModuleClass(SimpleModuleDefinition, ClassLoader, ModuleOptions)}
 * 
 * @author Mark Fisher
 */
public class TempModuleFactory extends ModuleFactory implements BeanClassLoaderAware, ResourceLoaderAware {

	private static Log log = LogFactory.getLog(TempModuleFactory.class);

	private volatile ClassLoader parentClassLoader = TempModuleFactory.class.getClassLoader();
	...
	public TempModuleFactory(ModuleOptionsMetadataResolver moduleOptionsMetadataResolver) {
		super(moduleOptionsMetadataResolver);
		this.moduleOptionsMetadataResolver = moduleOptionsMetadataResolver;
	}

	/**
	 * Public entry point to the method that we ultimately need to override just for the return value:
	 * {@link #determineModuleClass(SimpleModuleDefinition, ClassLoader, ModuleOptions)}
	 */
	@Override
